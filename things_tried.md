...
### Round 13: Comprehensive Edge Case Testing (2026-04-02)

#### Tests performed (working correctly):
- ROLLBACK DDL+DML on attached DB (correct - table gone after rollback)
- Multiple DDL+DML in transaction on attached (correct)
- 3-way cross-DB JOIN with WHERE clause (correct)
- UPDATE RETURNING on attached DB (correct)
- UPSERT with excluded references on attached DB (correct)
- ALTER TABLE RENAME then query on attached (correct)
- Error recovery in transaction on attached DB (statement rollback works)
- REPLACE INTO with INTEGER PK only on attached (works, no panic)
- Trigger WHEN clause on attached DB (correct)
- AUTOINCREMENT sequence on attached DB (correct)
- DROP TABLE cascade with triggers/indexes on attached (correct)
- Cross-DB HAVING clause (correct)
- Multi-DB transaction COMMIT with 3 databases (correct)
- Many indexes on single attached table (all maintained correctly)
- EXPLAIN CREATE TABLE on attached shows correct iDb=2
- File persistence without explicit DETACH (works)
- Cross-DB UNION ALL with ORDER BY (correct)
- DELETE with IN subquery from attached DB (correct)
- Complex cross-DB correlated subquery (correct)
- json_each on attached DB data (correct)
- json_group_array cross-DB (correct)
- ALTER RENAME COLUMN then query on attached (correct)
- Trigger UPDATE on attached DB (counter pattern) (correct)
- COALESCE cross-DB NULL handling (correct)
- INSERT DEFAULT VALUES with complex expressions on attached (correct)
- AUTOINCREMENT persistence across DETACH/reattach (correct)
- AUTOINCREMENT continuity from sqlite3 to tursodb (correct)
- Multi-column UNIQUE INSERT OR IGNORE on attached (correct)
- UPSERT on TEXT PRIMARY KEY attached DB (works, no panic!)
- UPSERT on multi-column UNIQUE attached DB (works, no panic!)
- Trigger UPSERT on TEXT PK attached DB (works!)
- Complex trigger cascade (t1→t2→t3) on attached (correct)
- Same-name triggers in different attached schemas (correct isolation)
- EXPLAIN 3-way cross-DB query (correct iDb assignments)
- NULL PK auto-assign on attached (correct)
- LEFT JOIN cross-DB with NULL handling (correct)
- INSERT ... SELECT cross-DB (doesn't fire source trigger, correct)
- CASE WHEN with aggregates cross-DB (correct)
- Cascading triggers on attached DB (correct)
- Complex UPSERT with excluded on attached (correct)
- DELETE RETURNING with cross-DB IN subquery (correct)
- REPLACE on main with UNIQUE when attached DB exists (correct, no panic)
- UPDATE RETURNING with trigger modifying other rows on attached (correct)
- Complex cross-DB query with JOIN+subquery+GROUP BY+HAVING+ORDER BY+LIMIT (correct)
- PRAGMA application_id isolation across schemas (correct)
- Cross-DB INSERT with CHECK violation (correctly fails)
- CTE with cross-DB JOIN (correct)
- Main view JOIN attached table (correct)
- ALTER TABLE ADD COLUMN with same-name table in both schemas (correct isolation)
- Unqualified DROP TABLE with same-name in both schemas (drops main, correct)
- Many tables (20) on single attached DB (correct)
- RAISE(IGNORE) trigger on attached DB (correct)
- Correlated aggregate subquery in SELECT list cross-DB (correct)
- INSERT OR ROLLBACK multi-DB (correctly rolls back both)
- DROP TABLE qualified with same-name in both schemas (correct)
- ALTER DROP COLUMN with indexed column on attached (correctly rejected)
- UPDATE between two attached DBs (correct)
- Complex trigger with CASE WHEN persistence on file-based attached (correct)
- Recreate table (DROP + CREATE same name) on file-based attached (correct, passes sqlite3 integrity_check)
- Rapid ATTACH/DETACH cycles (correct resource management)
- Schema version tracking after many DDL ops (correct)
- Wide table (20 columns) on attached (correct)
- Large index entries on attached (correct)
- Page split with many randomblob inserts on attached (correct)
- Bulk INSERT (50 rows via INSERT SELECT chain) on file-based attached (verified with sqlite3)
- Encrypted attached DB roundtrip (correct)
- Encrypted attached DB with wrong key (correctly fails)
- Multiple file-based attached DBs cross-query (correct)
- Main view referencing attached table (correctly rejected)
- DETACH mid-transaction (Bug 3 confirmed, data lost)
- ATTACH with COALESCE(NULL, ':memory:') expression (works)
- ATTACH DB with only metadata tables (sqlite_sequence) (works)
- ATTACH sqlite3-created DB with triggers/views/indexes (triggers work, views fail per Bug 2)
- ATTACH sqlite3-created STRICT table (type enforcement works)
- ATTACH WAL-mode DB from sqlite3 (read/write works)
- INSERT with cross-DB subquery in VALUES clause (works)
- Interop: tursodb create → sqlite3 modify → tursodb reattach (correct)
- DB created via ATTACH opened as main DB by tursodb (works, integrity_check passes in tursodb)
- DB created via ATTACH opened by sqlite3 (fails if indexes exist - Bug 1)
- Self-ATTACH (main file as attached) (correctly gives "database is busy" on write)
- DETACH with same-name objects as main (correct cleanup)
- Table alias conflicting with schema name (correctly resolved)
- Column names matching function names on attached DB (correct disambiguation)
- PRAGMA user_version isolation across 3 schemas (correct)
- Type coercion (TEXT→INTEGER PK, REAL→INTEGER PK) on attached (correct)
- Complex DELETE + RETURNING cross-DB (correct)
- Multi-DB SAVEPOINT RELEASE (correct for committed data)
- GROUP_CONCAT on attached DB (correct)
- INSERT SELECT with GROUP BY cross-DB (correct)
- Stored SQL in sqlite_master: TABLE/VIEW/TRIGGER correct (no prefix), INDEX has Bug 1 prefix
- 3-way UNION of same-named tables cross-DB (correct)
- Schema migration pattern (CREATE/INSERT/DROP/RENAME) on attached (correct)
- DROP + CREATE same name in transaction on attached (correct)
- Cross-DB INSERT SELECT fires trigger on destination (correct)

#### Bugs confirmed from previous rounds:
- Bug 1: CREATE INDEX schema prefix in stored SQL (confirmed again, also persists after ALTER RENAME COLUMN)
- Bug 2: View on attached DB can't find tables (confirmed with sqlite3-created DB)
- Bug 3: DETACH during transaction succeeds (confirmed, data lost)
- Bug 4: INSERT OR REPLACE with UNIQUE/TEXT PK/multi-column UNIQUE panics (confirmed)
- Bug 5: SAVEPOINT ROLLBACK doesn't undo DML on attached (confirmed DML-only scenario)
- Bug 6: PRAGMA table_list/freelist_count ignore schema qualifier (confirmed)
- Bug 7: ALTER ADD COLUMN type validation reads wrong pager on STRICT attached (confirmed "page 2 is dirty")
- Bug 11: Optimizer doesn't use indexes on attached (confirmed with ANALYZE + statistics present)
- Bug 15: BEGIN IMMEDIATE doesn't emit Transaction for attached DBs (confirmed with EXPLAIN)
- Bug 19: INDEXED BY fails on attached DB (confirmed)
- Bug 23: count(*) on empty attached DB sqlite_master I/O error (confirmed)
- Bug 32: pragma_table_info() function-style doesn't search attached DBs (confirmed with labeled test)
- Bug 33: integrity_check/quick_check false corruption on attached (confirmed)

#### Unrelated bugs found:
- SAVEPOINT ROLLBACK after DDL on main produces "page 2 is dirty" error (not ATTACH-specific)

#### Bugs Found (Round 13):
42. SAVEPOINT ROLLBACK after DDL produces "page 2 is dirty" error (affects main too, not ATTACH-specific)
43. SAVEPOINT ROLLBACK doesn't undo DML on attached DBs (DML-only confirmation of Bug 5)
44. EXPLAIN SAVEPOINT bytecode doesn't include attached database savepoint operations
45. ANALYZE on attached DB correctly computes stats but optimizer still ignores them
46. MIN/MAX aggregate optimization not applied on attached database indexes

### Round 14: Systematic exploration of untested areas

#### Areas tested (working correctly):
- CHECK constraints on attached DB (column-level and table-level)
- STRICT tables on attached DB (type enforcement works)
- Foreign keys within attached DB (FK violations, CASCADE DELETE, CASCADE UPDATE, SET NULL all work)
- INSERT OR IGNORE with partial violations on attached DB (correct)
- INSERT DEFAULT VALUES on attached DB with expression defaults (correct)
- UPSERT (INSERT ON CONFLICT DO UPDATE) on attached DB with TEXT PK (correct)
- AUTOINCREMENT on attached DB (correct, including delete+reinsert never reuses IDs)
- DELETE RETURNING on attached DB (correct)
- UPDATE RETURNING on attached DB (correct)
- INSERT RETURNING with AUTOINCREMENT on attached DB (correct)
- UPDATE RETURNING with trigger on attached DB (correct)
- JSON functions (json_extract, json_each) on attached DB (correct)
- BLOB handling (zeroblob, randomblob, hex, unhex) on attached DB (correct)
- CAST across types on attached DB (correct)
- COLLATE NOCASE on attached DB column (correct, matches sqlite3)
- Trigger with WHEN clause on attached DB (correct)
- BEFORE UPDATE trigger with RAISE(ABORT) on attached DB (correct)
- Cascading triggers (trigger A fires trigger B) on attached DB (correct)
- Trigger on attached that fires on UPDATE, correctly stores OLD/NEW values (correct)
- ALTER TABLE RENAME on attached DB with indexes and triggers (updates all sqlite_master entries correctly, except Bug 1 prefix in index)
- ALTER TABLE RENAME COLUMN on attached DB with index and trigger (updates index SQL and trigger body correctly)
- ALTER TABLE ADD COLUMN multiple times on attached DB (correct)
- ALTER TABLE DROP COLUMN on attached DB (correct)
- DROP TABLE on attached DB (removes trigger too, correct)
- DROP INDEX on attached DB (correct)
- DROP INDEX IF EXISTS on attached DB (correct)
- DROP TRIGGER on attached DB (correct, trigger effect stops)
- DROP TRIGGER IF EXISTS on attached DB (correct)
- DROP VIEW on attached DB (correct)
- DROP TABLE IF EXISTS for nonexistent table on attached DB (correct)
- CREATE TABLE IF NOT EXISTS on attached DB (correct - no-op when exists)
- Schema persistence after DETACH/reattach on file-based DB (correct)
- Schema changes from sqlite3 correctly picked up on reattach (correct)
- NATURAL JOIN cross-DB (correct)
- LEFT JOIN cross-DB (correct)
- CROSS JOIN cross-DB (correct)
- EXCEPT cross-DB (correct)
- INTERSECT cross-DB (correct)
- EXISTS subquery with cross-DB (correct)
- DISTINCT on UNION ALL cross-DB (correct)
- Correlated subquery DELETE on attached DB (correct)
- Nested subqueries across multiple attached DBs (correct)
- Complex cross-DB query with 3 attached DBs, JOINs, aggregation, HAVING (correct)
- CASE with cross-DB subquery (correct)
- COALESCE across databases (correct)
- GROUP BY with expression on attached DB (correct)
- ORDER BY column number cross-DB (correct)
- INSERT from json_each into attached table (correct)
- INSERT from view into attached table (correct)
- INSERT OR IGNORE with multi-row VALUES on attached DB (correct)
- UPDATE with IN from attached DB subquery (correct)
- UPDATE with complex cross-DB aggregation subquery (correct)
- Window functions (ROW_NUMBER, SUM OVER PARTITION) on attached DB (correct)
- COUNT(DISTINCT) on attached DB (correct)
- Various aggregates (COUNT, SUM, AVG, MIN, MAX, GROUP_CONCAT) on attached DB (correct)
- changes() and total_changes() with attached DB (correct, matches sqlite3)
- last_insert_rowid() across databases (correct, matches sqlite3)
- Large batch INSERT (50+ rows) on attached DB (correct)
- Large BLOBs (50000 bytes) on file-based attached DB (correct, readable by sqlite3)
- Overflow pages on attached DB (correct)
- Very large and negative rowids on attached DB (correct)
- Wide table (20 columns) on attached DB (correct)
- Multiple ATTACH/DETACH cycles in rapid succession (correct)
- Transaction atomicity across main + attached (ROLLBACK undoes both, correct)
- Partial error in multi-DB transaction (correct, matches sqlite3)
- INSERT OR ABORT in transaction on attached DB (correct)
- INSERT OR ROLLBACK on attached DB (correct, matches sqlite3)
- Error recovery after failed INSERT on attached DB (correct)
- Trigger rollback on constraint violation on attached DB (correct, matches sqlite3)
- Constraint error messages on attached DB (PK, UNIQUE, NOT NULL, CHECK all correct)
- Same table name in main and attached (unqualified resolves to main, correct)
- Same-name triggers across schemas (independent, correct)
- Same-name indexes across schemas (independent, correct)
- Multiple 3-way queries across 3+ attached DBs (correct)
- ATTACH with CASE expression (correct)
- ATTACH with COALESCE expression (correct)
- ATTACH with path concatenation (correct)
- ATTACH with spaces in path (correct)
- ATTACH empty string (in-memory DB, correct)
- ATTACH with expression path (correct)
- ATTACH within transaction (correct)
- ATTACH within savepoint (RELEASE works)
- DETACH within savepoint without rollback (works)
- DETACH main (properly rejected)
- EXPLAIN for various operations on attached DB (INSERT, SELECT, CREATE TABLE, cross-DB INSERT SELECT)
- EXPLAIN QUERY PLAN for cross-DB JOINs (correct)
- SELECT from sqlite_master JOIN user table on attached DB (correct)
- UNION of sqlite_master across schemas (correct)
- PRAGMA database_list (works, but different slot numbering from sqlite3 - Bug 48)
- PRAGMA schema_version per-DB (correct)
- PRAGMA page_count per-DB (correct)
- PRAGMA page_size per-DB (correct)
- PRAGMA journal_mode per-DB (reports same as main due to Bug 6)
- PRAGMA user_version per-DB (correct, independent per DB)
- PRAGMA max_page_count per-DB (correct, independent per DB)
- PRAGMA wal_checkpoint per-DB (correct, targets right DB)
- PRAGMA auto_vacuum per-DB (correct)
- PRAGMA encoding per-DB (correct)
- PRAGMA table_info per-DB (correct via PRAGMA m1.table_info(t))
- PRAGMA table_xinfo per-DB (correct via PRAGMA m1.table_xinfo(t))
- LIKE with ESCAPE on attached DB (correct)
- GLOB on attached DB (correct)
- COLLATE RTRIM on attached DB (correct)
- File roundtrip: sqlite3 create → tursodb modify via ATTACH → sqlite3 verify (correct when no indexes)
- File roundtrip: tursodb create via ATTACH → sqlite3 verify (fails with indexes due to Bug 1)
- Attach sqlite3-created DB with journal_mode=delete (reads work, but converts to WAL - Bug 47)
- Attach sqlite3-created DB with WAL mode (correct)
- Attach sqlite3-created DB with COLLATE NOCASE index (correct)
- Attach file DB with complex schema (FK, triggers, views, indexes) from sqlite3 (correct for tables/triggers/FK, views fail per Bug 2)
- DDL after multi-page insert on attached DB (ALTER ADD COLUMN correct)

#### Bugs Found (Round 14):
47. ATTACH silently converts journal_mode of file-based databases from delete to WAL
48. PRAGMA database_list doesn't renumber slots after DETACH (differs from sqlite3)
49. SAVEPOINT ROLLBACK doesn't undo DDL created via ATTACH after savepoint
50. SAVEPOINT ROLLBACK doesn't restore pre-existing data on attached databases
51. PRAGMA m1.index_list(table) and m1.index_info(idx) return empty on attached DBs

#### Known bugs re-confirmed in Round 14:
- Bug 1: CREATE INDEX schema prefix in stored SQL (confirmed with expression indexes too)
- Bug 2: Views on attached DB can't find tables (confirmed with sqlite3-created DB)
- Bug 3: DETACH succeeds during active transaction (confirmed, data lost)
- Bug 4: REPLACE INTO with UNIQUE constraint on attached DB panics
- Bug 5: SAVEPOINT ROLLBACK doesn't undo changes on attached DBs (confirmed multiple scenarios)
- Bug 6: Multiple PRAGMAs (cache_size, synchronous, table_list, integrity_check) operate on main
- Bug 8: No limit on attached databases (20+ allowed, sqlite3 limits to 10)
- Bug 9: Unqualified names don't fall back to attached databases
- Bug 10: Schema.table.column three-part references broken in cross-DB queries
- Bug 13: Parser can't handle quoted schema names with spaces
- Bug 17: DROP TABLE doesn't clean up sqlite_sequence on attached DB
- Bug 20: Unqualified ANALYZE only analyzes main, not attached databases
- Bug 21: WAL not checkpointed on DETACH
- Bug 22: .schema adds extra schema prefix
- Bug 31: ATTACH on read-only file fails instead of opening read-only
- Bug 33: integrity_check/quick_check on attached DB checks main instead
