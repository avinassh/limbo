...
### 40. VACUUM fails on SQLite-created DB with double-quoted string literal in CHECK/GENERATED
Tested a variety of database shapes generated by SQLite where the schema
uses double-quoted strings in DDL expression contexts (CHECK constraints,
GENERATED VIRTUAL column expressions, LIKE/GLOB patterns within CHECK).
SQLite preserves double-quoted strings as literals in DDL for backwards
compat; Turso's CREATE TABLE replay treats them as identifier references
and bails with `no such column: <literal>`. VACUUM succeeds on the same
DB only when the DDL uses single-quoted strings. Logged as Bug 34.

### 41. Other things tried, no new bug
- VACUUM + AUTOINCREMENT table that was RENAMEd then VACUUM'd — sqlite_sequence
  retains renamed name (matches SQLite)
- VACUUM + source has CREATE TABLE with DEFAULT using built-in hex()/printf() —
  value preserved; SQL re-stringified but semantically equivalent
- VACUUM + DESC INTEGER PRIMARY KEY + sqlite_autoindex — preserved (matches SQLite)
- VACUUM + PRAGMA locking_mode=EXCLUSIVE (per-conn) — no effect on VACUUM
- VACUUM + extensive ALTER TABLE chain (ADD/RENAME/DROP columns) — schema consistent
- VACUUM + source has JSON content with jsonb() — byte-preserving
- VACUUM + triggers with cascade behavior (insert into A fires trigger to B,
  trigger on B fires to C) — all preserved, don't fire during copy
- VACUUM INTO when dest dir is a symlink to a real dir — created in real dir
- VACUUM INTO when dest is a symlink to an EXISTING regular file — rejected
- VACUUM with source that has very LONG column names (1000+ chars) — preserved
- VACUUM with large BLOB (1MB) requiring overflow pages — preserved
- VACUUM + source has sqlite_sequence with ALL unusual seq types (BLOB, TEXT,
  FLOAT, NULL) — non-integer seq overwritten to max(rowid) (Bug 6 family)
- VACUUM + source has MVCC with custom __turso_internal_mvcc_meta value —
  target starts fresh (persistent_tx_ts_max=2)
- VACUUM + source has sqlite_schema entry for VIEW that has column alias list —
  preserved (sql column format unchanged)
- VACUUM + source has WINDOW function in VIEW — preserved
- VACUUM + source has CTAS (CREATE TABLE AS SELECT) with JOIN — stored SQL
  reformatted post-VACUUM (U3 family, expected)
- VACUUM INTO + source is mvcc + uncheckpointed writes — dest correctly
  captures logical state (vs in-place which requires checkpoint)
- VACUUM + source's CREATE TABLE has deeply nested CASE/CAST/AND/OR (up to ~80
  clauses) — stack overflow during replay (U8 family). SQLite rejects at parse
  for extremely deep trees (depth 1000+); Turso hits overflow at much lower depth
- VACUUM + NUMERIC PRIMARY KEY (stored as rowid) — preserved
- VACUUM + compound PRIMARY KEY rowid preservation — matches SQLite
- VACUUM + CREATE INDEX with COLLATE BINARY/NOCASE in WHERE clause — Turso
  rejects at replay ("cannot use ... in WHERE clause of CREATE INDEX"). This
  is an extension of Bug 28 where Turso's CREATE INDEX parser is stricter
  than SQLite's. The Bug 28 log specifically calls out non-deterministic
  functions; COLLATE clauses hit the same code path but are conceptually
  different. Noted as part of Bug 28 rather than a separate entry.
- VACUUM + source has CHECK using sqlite_log() (SQLite-only function) — Turso
  fails at schema replay with "no such function: sqlite_log". Not strictly a
  VACUUM bug since INSERT hits the same error; same root cause as Bug 34
  pattern (strict replay).
- VACUUM + source's schema has CREATE TRIGGER body with double-quoted strings
  — VACUUM succeeds (trigger body is not compiled at replay). Subsequent
  INSERTs into the triggering table fail with "no such column: <literal>"
  because trigger compiles at fire time. Similar root cause as Bug 34 but
  surfaces differently. Logged under Bug 34's "how to reproduce".
- VACUUM + encrypted+MVCC source — reopen fails with U7-family error (CLI
  eager read before PRAGMA cipher). VACUUM INTO attempts error the same way.
- VACUUM INTO + dest is a dangling symlink — writes through to the symlink
  target path (if parent dir exists); otherwise fails with U4/U11 error
- VACUUM + PRAGMA schema_cookie near u32::MAX — wraps to 0 on VACUUM (matches
