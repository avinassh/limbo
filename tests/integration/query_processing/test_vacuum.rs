...
fn compute_plain_vacuum_dbhash(tmp_db: &TempDatabase) -> String {
    let options = turso_dbhash::DbHashOptions {
        without_schema: true,
        ..Default::default()
    };
    compute_dbhash_with_options_and_database_opts(tmp_db, &options, tmp_db.db_opts).hash
}

fn compute_plain_vacuum_file_dbhash(path: &str) -> String {
    let options = turso_dbhash::DbHashOptions {
        without_schema: true,
        ..Default::default()
    };
    turso_dbhash::hash_database(path, &options)
...
fn assert_plain_vacuum_integrity_and_hash_for_path(
    ...
    assert_eq!(run_integrity_check(conn), "ok");
    assert_eq!(
        compute_plain_vacuum_file_dbhash(path),
        expected_hash,
        "plain VACUUM should preserve logical database content"
    );
}
...
fn test_mvcc_plain_vacuum_active_read_tx_returns_busy() -> anyhow::Result<()> {
    ...
    assert_plain_vacuum_integrity_and_hash(&tmp_db, &writer, &before_hash);
