...
fn shuttle_test_lost_updates() {
    let scheduler = RandomScheduler::new(1000);
    let runner = shuttle::Runner::new(scheduler, shuttle_config());
    runner.run(|| shuttle::future::block_on(lost_updates_scenario(3, 1)));
}

#[test]
fn shuttle_test_lost_updates_multi_round() {
...
#[test]
fn shuttle_test_lost_updates_full() {
    let scheduler = RandomScheduler::new(100);
    let runner = shuttle::Runner::new(scheduler, shuttle_config());
    // runner.run(|| shuttle::future::block_on(lost_updates_scenario(16, 100)));
    runner.run(|| shuttle::future::block_on(lost_updates_scenario(8, 40)));
}
