...
impl ChaoticElleWorkload {
    fn new(table_name: String, model: ElleModelKind, ops: Vec<PlannedOp>) -> Self {
        Self {
            table_name,
            model,
            ops,
...
impl ChaoticWorkload for ChaoticElleWorkload {
    fn next(&mut self, result: Option<OpResult>) -> Option<Operation> {
        ...
        let planned = &self.ops[self.index].clone();
        self.index += 1;
...
impl ChaoticElleProfile {
    pub fn new(table_name: String, model: ElleModelKind, value_counter: Arc<AtomicI64>) -> Self {
        Self {
            table_name,
            model,
            value_counter,
