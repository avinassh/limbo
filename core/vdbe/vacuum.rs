...
mod tests {
    ...
    impl SyncCountingIo {
        ...
        fn db_syncs(&self) -> usize {
            self.db_syncs.load(StdOrdering::SeqCst)
        }
    }
    ...
    impl IO for SyncCountingIo {
        fn open_file(&self, path: &str, flags: OpenFlags, direct: bool) -> Result<Arc<dyn File>> {
            ...
            Ok(Arc::new(SyncCountingFile {
                inner,
                count_as_db_sync: !path.ends_with("-wal"),
    ...
    fn finalize_vacuum_into_output_syncs_destination_db_file() -> Result<()> {
        let io = Arc::new(SyncCountingIo::new());
        ...
        assert!(
            io.db_syncs() > 0,
            "VACUUM INTO finalization must sync the destination db file before success"
        );
